...
/*
Mapping of temporaries and variables to registers:
t0..t9 -> $t0..$t9
t10+ -> $s0..$s7 (limited)
variables -> in the stack (lw/sw)
*/

// helpers
// returns var offset of respective var stores in symbol table
...
{
    int num_regs = 16;
    int index = temp_id % num_regs;
    static char buf[10];
    if (index < 8)
    {
        snprintf(buf, sizeof(buf), "$t%d", index + 2);
    }
    else
    {
        int s_idx = index - 8;
        snprintf(buf, sizeof(buf), "$s%d", s_idx);
    }
    return strdup(buf);
}
...
// print MIPS .data section to output file
...
// prints li/addi or lui/ori for constants to output file
...
static inline void reset_int_reg_cursor(void)
{
    regs_idx = 0;
}

// returns next register in round-robin style.. used by load_atom_to_reg() to select next register to use
static const char *next_int_reg(void)
{
    const int N = (int)sizeof(regs) / sizeof(regs[0]);
    const char *r = regs[regs_idx % N];
    regs_idx++;
    return r;
}

// load atom (temp/var/const) into a register
...
// store register into destinationn (temp -> move to temp reg / var -> store into stack mem $fp + offset)
...
// generate MIPS code for binop (math/logical operations)
static void emit_binop(FILE *out, const char *dest, const Atom *a1, binop bop, const Atom *a2, Table table)
{
    ...
    char *rd = strdup("$t1"); // use $t1 as temp register for result
    char *base = NULL;
    char *exp = NULL;
    char *acc = NULL;
    switch (bop)
...
// generates MIPS comparison instructions that brank to labelT or labelF
...
generate all MIPS instructions for program
generate epilogue
 */
void printMIPS(const InstrList *list, Table table, FILE *out)
{
    ...
    for (InstrNode *n = list->head; n; n = n->next)
    {
        ...
        if (i->iop == OP_PRINT)
        {
            ...
            {
                (void)pool_string_literal(i->arg1.contents.sval ? i->arg1.contents.sval : "");
            }
        ...
        else if (i->iop == OP_ASSIGN)
        {
            ...
            if (i->has_binop)
            {
                ...
                {
                    (void)pool_string_literal(i->arg2.contents.sval ? i->arg2.contents.sval : "");
                }
        ...
        else if (i->iop == OP_COND)
        {
            ...
            if (i->rel_left.kind == ATOM_FLOAT)
                (void)pool_float_literal(i->rel_left.contents.fval);
            if (i->rel_right.kind == ATOM_STRING)
                (void)pool_string_literal(i->rel_right.contents.sval);
            if (i->rel_right.kind == ATOM_FLOAT)
                (void)pool_float_literal(i->rel_right.contents.fval);
        }
        else if (i->iop == OP_READ)
        {
            const char *name = i->dest ? i->dest : "";
            SymbolInfo *sinfo = lookup_value(table, (char *)name);
            if (sinfo && (sinfo->type == T_STRING))
            {
                // string read
                g_emit_default_str_buf = 1;
            }
        }
    }
    ...
    for (InstrNode *n = list->head; n; n = n->next)
    {
        ...
        case OP_JUMP_FALSE:
        {
            ...
            {
                fprintf(out, "    # Warning: JUMP_FALSE with float not supported\n");
                break;
            }
        ...
        case OP_PRINT:
        {
            ...
            {
                const char *rf = pool_float_literal(instr->arg1.contents.fval);
                fprintf(out, "    la $t0, %s\n", rf);
                fprintf(out, "    lwc1 $f12, 0($t0)\n");
                fprintf(out, "    li $v0, 2\n");
                fprintf(out, "    syscall\n");
            }
            else if (instr->arg1.kind == ATOM_TEMP)
            {
                char *rv = load_atom_to_reg(out, &instr->arg1, table);
                fprintf(out, "    move $a0, %s\n", rv);
                fprintf(out, "    li $v0, 1\n");
            ...
            else if (instr->arg1.kind == ATOM_VAR)
            {
                const char *name = instr->arg1.contents.name ? instr->arg1.contents.name : "";
                SymbolInfo *sinfo = lookup_value(table, (char *)name);
                if (sinfo && (sinfo->type == T_STRING))
                {
                    int offset = getVarOffset(table, name);
                ...
                else if (sinfo && sinfo->type == T_FLOAT)
                {
                    int offset = getVarOffset(table, name);
                    fprintf(out, "    lwc1 $f12, %d($fp)\n", offset);
                    fprintf(out, "    li $v0, 2\n");
                    fprintf(out, "    syscall\n");
                }
                else
        ...
        case OP_READ:
        {
            ...
            if (sinfo && sinfo->type == T_FLOAT)
            {
                fprintf(out, "    li $v0, 6\n");
                fprintf(out, "    syscall\n");
                fprintf(out, "    swc1 $f0, %d($fp)\n", offset);
            }
            else if (sinfo && (sinfo->type == T_STRING))
            {
                g_emit_default_str_buf = 1;
                fprintf(out, "    la $a0, str_buf\n");
                fprintf(out, "    li $a1, %d\n", DEFAULT_STR_BUF_SIZE);
                fprintf(out, "    li $v0, 8\n");
                fprintf(out, "    syscall\n");
                fprintf(out, "    la $t0, str_buf\n");
...
// creates a MIPS file in which it places the mips code generated by printMIPS(list, table, f)
